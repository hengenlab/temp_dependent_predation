...
# df_subset = pd.read_csv('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_ckbn.csv') # , nrows=100)
df_subset = pd.read_csv('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_ckbn_final.csv') # , nrows=100)
...
for id_sub_i in df_subset.id.unique():
    ...
    if len(df_local) > 0:
        ...
        # pretrial make dt_start as dt_end tthen calculate dt_start is dt_start - 60seconds
        dt_end = None
        dt_end = dt_start
        dt_start = dt_start - timedelta(hours=0, minutes=0, seconds=60)
        ...
        if not os.path.exists('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_temphum_ckbn_final.csv'):
            print("creating file", flush=True)
            # tmp_df_temp_trails.to_csv('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_temphum_ckbn_final.csv', mode = 'w', sep = ',', encoding='utf-8', header=True, index=False)
            with open('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_temphum_ckbn_final.csv', 'w') as file:
                tmp_df_temp_trails.to_csv(file, sep=',', encoding='utf-8', header=True, index=False)
        else:
            print("appending to file", flush=True)
            # tmp_df_temp_trails.to_csv('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_temphum_ckbn_final.csv', mode = 'a', sep = ',', encoding='utf-8', header=False, index=False)
            with open('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_pretrial_dlc_features_all_temphum_ckbn_final.csv', 'a') as file:
