            ...
            for index, row in df_new.iterrows():
                ...
                if ((isinstance(row.start_time_cap, str)) and (isinstance(row.end_time_cap, str))):
                    ...
                    if not((row.start_time_cap.isspace()) or (row.end_time_cap.isspace())):
                        ...
                        if not os.path.exists('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_dlc_features_all_ckbn_final.csv'):
                            print("creating file", flush=True)
                            # df_feat_sub_range.to_csv('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_dlc_features_all_ckbn_final.csv', mode = 'w', sep = ',', encoding='utf-8', header=True, index=False)
                            with open('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_dlc_features_all_ckbn_final.csv', 'w') as file:
                                df_feat_sub_range.to_csv(file, sep=',', encoding='utf-8', header=True, index=False)
                        else:
                            print("appending to file", flush=True)
                            # df_feat_sub_range.to_csv('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_dlc_features_all_ckbn_final.csv', mode = 'a', sep = ',', encoding='utf-8', header=False, index=False)
                            with open('/media/HlabShare/ckbn/Prey_Capture_outputs/capture_dlc_features_all_ckbn_final.csv', 'a') as file:
